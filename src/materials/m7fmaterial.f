...
C +--------------------------------------------------------------------+
     ...
     $snf_no_split,E_N,zeta) 
      ...
      double precision :: young, poisson, k_1, C0,dtime,th_del,unit_conv
     ...
     $     c_17,c_18,c_19,c_20,c_21,C_R0,C_R2
      double precision :: k_2, k_3, k_4, k_5, k_6, k_7, k_8, k_9, sv0_p
      double precision :: c_1, c_2, c_3, c_4, c_5, c_6, c_7, c_8,
     $     c_9, c_10
      double precision :: c_11,c_12,c_13,c_14,c_15,c_16,c_17,c_18,
     ...
     $     d_5, d_6
      ...
      double precision :: cf_4, cf_5, cf_4_4
      double precision :: ap1, bp1, cp2, dp2, ep2
...
! +--------------------------------------------------------------------+
     ...
     $     d_5, d_6
      ...
      save :: /KCommon_Vars/, /KM7fBounds_M7fIO/, /KM7fBounds_M7fIO_1/,
     $     /KM7fBounds_M7fIO_2/
      double precision, dimension(1:np) :: ded, ed0, sd0
      double precision, dimension(1:np) :: sdf, C_d

      double precision, dimension(1:np) :: sdneg, sdpos

      double precision :: dev, ev0, sv0, R_D
      double precision :: usi_C, usi_T, par5, par6, E_s, Cd0, Ev
      double precision, dimension(1:np) :: edf, sde, sdlower, sdupper
      double precision, dimension(1:np) :: sdlower_fib, sdupper_fib
...
C +--------------------------------------------------------------------+
     ...
     $     /KM7fBounds_M7fIO_2/
      ...
      double precision :: svneg 
      double precision :: R_N 
      double precision :: Cv0, evf, svb_neg 
      double precision :: xk0, e0_V

      double precision :: prStrainDiff, xk4 
...
C **********************************************************************
     ...
     $     deps_L, deps_M, eps_N,  eps_L,  eps_M,  sig_N
      double precision, dimension(1:np)     :: sd0, sn0,
     $     ded, ed0, edf
      double precision, dimension(1:np)     :: enf, snf,
     $     snb, sdf, sdneg, sdpos, snb_fib
      double precision, dimension(1:np)     :: sl0, sm0, slf,
     $     smf, del, dem, eps_N0_neg, eps_N0_pos
      double precision, dimension(1:nvhi+2)    :: vh_ini, vh_fin 
      double precision, dimension(1:np)     :: C_d, E_N
      double precision :: Cv
      save    :: /KCommon_Vars/
...
C
     ...
     $     old_dtime, lruncreep, lfirstpass 
      double precision :: StrainRateTensorInv2,
     $     DevStrainRateTensorInv2, old_dtime
      ...
      double precision :: C_R1, q_1  
      double precision :: t_0, t_ch, f_c, a_to_c, g_c, w_to_c, E28
      common /KM7f_Microplane_1/ qn, ql, qm, w 
      common /KM7f_Microplane_2/ deps_N, deps_L, deps_M, eps_N,  eps_L,
     $     eps_M,  sig_N
      common /KM7f_Microplane_3/ sd0, sn0, ded, ed0, edf, enf, snf, snb,
     ...
     $     /KM7f_Microplane_3/, /KM7f_Microplane_4/, /KM7f_Microplane_5/
      ...
      double precision :: R_N, dfb
      double precision :: sv0, ev0, dev, evf, sum_snf, devv
      double precision :: svfB, svf, phi0, phi, sig_I_old, eps_I
      integer ::  jp, i 
      double precision :: svneg, sum1, strain_rate, ek(1:6,1:6)
...
C+---------------------------------------------------------------------+
      ...
      double precision :: t_0, t_ch, f_c, a_to_c, g_c, w_to_c, E28
      double precision :: dt
      double precision, parameter  :: DaysPerSec = 1.1574074074074074d-5 
      double precision :: t_age, t_ch_p, density, q1_opt
      double precision :: q1, q2, q3, q4, n, m, t, J, r, Z, Qf, Q, J_dot
      double precision :: Ep, eta, tau1, C1
...
C **********************************************************************
      subroutine inputparams(E, nu, k1, k2, k3, k4) 
      use, intrinsic :: iso_fortran_env, only: RK => real64
      double precision :: E, nu, k1, k2, k3, k4
      double precision, parameter :: PI=3.1415926535897932384626433832d0
      integer, parameter :: np=37, nvhm=5, nvhf=2, nvhi=np*nvhm+nvhf
      common /KCommon_Vars/young, poisson,k_1, C0,dtime,th_del,unit_conv
      double precision :: young, poisson, k_1, C0,dtime,th_del,unit_conv
